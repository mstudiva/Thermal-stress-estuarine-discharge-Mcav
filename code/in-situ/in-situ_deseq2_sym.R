...
#read in counts
counts = read.table("insitu_allcounts_sym.txt")
...
vsd=assay(Vsd)
# takes the factor levels from the design to create new column names for the dataframe
...
pdf(file="heatmap_insitu_sym.pdf", width=10, height=10)
pheatmap(cor(vsd))
dev.off()
...
pdf(file="PCoA_insitu_site_sym.pdf", width=12, height=6)
par(mfrow=c(1,2))
...
pdf(file="PCoA_insitu_season_sym.pdf", width=12, height=6)
par(mfrow=c(1,2))
...
pdf(file="Venn_insitu_sym.pdf", height=12, width=12)
...
pdf(file="Venn_insitu_sym_pairwise.pdf", height=12, width=12)
