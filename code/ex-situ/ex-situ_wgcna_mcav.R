...
a=as.numeric(design$colony=="a")
b=as.numeric(design$colony=="b")
c=as.numeric(design$colony=="c")
d=as.numeric(design$colony=="d")
e=as.numeric(design$colony=="e")
f=as.numeric(design$colony=="f")
g=as.numeric(design$colony=="g")
h=as.numeric(design$colony=="h")
i=as.numeric(design$colony=="i")
j=as.numeric(design$colony=="j")
k=as.numeric(design$colony=="k")
# control=as.numeric(design$temp=="control")
# elevated=as.numeric(design$temp=="elevated")
# offshore=as.numeric(design$water=="offshore")
# discharge=as.numeric(design$water=="discharge")
control.offshore=as.numeric(design$temp.water=="control.offshore")
control.discharge=as.numeric(design$temp.water=="control.discharge")
elevated.offshore=as.numeric(design$temp.water=="elevated.offshore")
elevated.discharge=as.numeric(design$temp.water=="elevated.discharge")

traits <- cbind(a,b,c,d,e,f,g,h,i,j,k,control.offshore,control.discharge,elevated.offshore,elevated.discharge, design[c(9:14)])
# just the quantitative data (symbiont and chlorophyll metrics)
...
barplot(meanExpressionByArray,
        ...
        names.arg = c(1:127), cex.names = 0.7)
...
# samples 1, 3, and 4 found to be outliers
...
s.th=11 # re-specify according to previous section
...
# good way to do it is to find a group of similar modules in the heat map and then see at which tree height they connect in the dendrogram.
...
# whichTrait="i"
# whichTrait="k"
# whichTrait="control.offshore"
# whichTrait="control.discharge"
# whichTrait="elevated.offshore"
whichTrait="elevated.discharge"
...
# moduleCols=c("yellow","green","blue","brown") # for c
# moduleCols=c("turquoise") # for d
# moduleCols=c("green","blue") # for e
# moduleCols=c("green","blue") # for h
# moduleCols=c("green") # for i
# moduleCols=c("green") # for k
# moduleCols=c("turquoise") # for control.offshore
# moduleCols=c("blue") # for control.discharge
# moduleCols=c("yellow") # for elevated.offshore
moduleCols=c("turquoise","yellow","green") # for elevated.discharge
...
# par(mfrow=c(1,4)) # for c 
# par(mfrow=c(1,1)) # for d
# par(mfrow=c(1,2)) # for e
# par(mfrow=c(1,2)) # for h
# par(mfrow=c(1,1)) # for i
# par(mfrow=c(1,1)) # for k
# par(mfrow=c(1,1)) # for control.offshore
# par(mfrow=c(1,1)) # for control.discharge
# par(mfrow=c(1,1)) # for elevated.offshore
par(mfrow=c(1,3)) # for elevated.discharge
counter=0
...
# dev.off()
# pdf(file="heatmap_top30_yellow.pdf", height=6, width=22)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F)
# dev.off()
# pdf(file="heatmap_top30_green.pdf", height=6, width=22)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F)
# dev.off()
# pdf(file="heatmap_top30_blue.pdf", height=6, width=32)
# pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F)
# dev.off()
pdf(file="heatmap_top30_brown.pdf", height=6, width=24)
pheatmap(hubs,scale="row",col=contrasting2,border_color=NA,treeheight_col=0,cex=0.9,cluster_rows=F)
dev.off()
