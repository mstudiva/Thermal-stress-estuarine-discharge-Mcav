...
#read in counts
counts = read.table("exsitu_allcounts_mcav.txt")
...
vsd=assay(Vsd)
# takes factor levels from the design to create new column names for the dataframe
...
pdf(file="heatmap_exsitu_mcav.pdf", width=25, height=25)
pheatmap(cor(vsd))
dev.off()
...
pdf(file="PCoA_exsitu_genotype_mcav.pdf", width=12, height=6)
par(mfrow=c(1,2))
...
legend("bottomright", legend=c("a","b","c","d","e","f","g","h","i","j","k"), fill = c("#40004b","#762a83", "#9970ab","#c2a5cf","#e7d4e8","#f7f7f7", "#d9f0d3","#a6dba0","#a6dba0","#1b7837", "#00441b"), bty="n")
legend("topright", legend=c("control.offshore", "control.discharge","elevated.offshore","elevated.discharge"), pch=c(0,15,2,17), bty="n")
...
legend("topright", legend=c("control.offshore", "control.discharge","elevated.offshore","elevated.discharge"), fill = c("#92c5de","#0571b0","#f4a582","#ca0020"), bty="n")
legend("bottomright", legend=c("a","b","c","d","e","f","g","h","i","j","k"), pch=c(1,3,4,5,6,7,8,9,10,16,18), bty="n")
dev.off()

# plotting PCoA by treatments separately
pdf(file="PCoA_exsitu_mcav.pdf", width=12, height=6)
par(mfrow=c(1,2))
...
legend("bottomright", legend=c("control","elevated"), fill = c("#0571b0","#ca0020"), bty="n")
legend("topright", legend=c("offshore","discharge"), pch=c(0,15), bty="n")
...
legend("topright", legend=c("offshore","discharge"), fill = c("#018571","#a6611a"), bty="n")
legend("bottomright", legend=c("control","elevated"), pch=c(25,17), bty="n")
dev.off()
...
# 40 PCs
xvalDapc(t(vsd.comb[,conditions$temp.water!="elevated.discharge"]),conditions$temp.water[conditions$temp.water!="elevated.discharge"], n.rep=1000, n.pca=30:50, parallel="multicore", ncpus= 8)
# 48 PCs

# now running the dapc without elevated.discharge samples
dp.tw=dapc(t(vsd.comb[,conditions$temp.water!="elevated.discharge"]),conditions$temp.water[conditions$temp.water!="elevated.discharge"],n.pca=48, n.da=2)
dp.tw$ind.coord
...
dp.ed$grp<-as.factor(c("elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge","elevated.discharge"))

# now exporting side by side figures of control vs single-factor treatments and double-factor treatment
pdf(file="DAPC_exsitu_mcav.pdf", width=12, height=6)
par(mfrow=c(1,2))
...
write.csv(dpc.tw, "DAPC_exsitu_mcav_DFA.csv", quote=F)
# modify the output CSV to add in a column for temp.water

# then reimport
dpc.tw<- read.csv("DAPC_exsitu_mcav_DFA.csv")
...
pdf(file="Venn_exsitu_mcav.pdf", height=10, width=12)
...
pdf(file="Venn_exsitu_mcav_pairwise.pdf", height=6, width=8)
