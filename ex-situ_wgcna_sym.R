...
traits <- cbind(a,b,c,d,e,f,g,h,i,j,k,control.offshore,elevated.offshore,control.discharge,elevated.discharge, design[c(9:14)])
# just the quantitative traits (symbiont/chlorophyll metrics)
...
barplot(meanExpressionByArray,
        ...
        names.arg = c(1:125), cex.names = 0.7)
...
# samples 1, 2, 3, 124, and 125 found to be outliers
...
# in this case, SFT.R.sq never went above 0.90, but I picked the highest (s.th=4)
...
table(dynamicColors)
# NOT ENOUGH GENES, ONLY ONE MODULE FOUND
# DO NOT PROCEED
...
# if it was first pass with no module merging, this is where you examine your heatmap and dendrogram of module eigengenes to seewhere you would like to set cut height (MEDissThres parameter) in the previous section to merge modules that are telling the same story for your trait data
...
moduleCols=c("yellow","green","blue","brown") # example

quartz()
par(mfrow=c(1,4)) # set par to be big enough for all significant module correlations, then run the next whichTrait and moduleCols statements above and repeat from the 'for' loop
counter=0
